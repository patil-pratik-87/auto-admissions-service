#!/bin/bash
# Regenerate the rule-based baseline: one fresh `admissions screen` run per persona
# (digital PDFs only), written to baseline/<persona>/ with full logs.
# Idempotent: personas with an existing application-result.json are skipped.
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$HERE/../.." && pwd)"
BASE="$ROOT/alternate-options/option-b-full-policy-workflow/baseline"
cd "$ROOT" || exit 1
